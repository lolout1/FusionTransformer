...
\textbf{Computational efficiency.} Our model requires approximately 73,400 parameters, a compact footprint suitable for resource-constrained wearable devices. The simple architecture (Conv1D projections, 2-layer transformer, linear classifier) avoids operations that are expensive on embedded hardware (e.g., recurrent layers, large attention spans). Quantization to INT8 precision would further reduce memory and latency for deployment.
