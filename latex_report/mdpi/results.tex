...
Interestingly, dual-stream architecture with raw gyroscope input shows \textit{decreased} performance ($-$1.38\% F1) compared to single-stream. This counterintuitive result has an important explanation: raw gyroscope signals are inherently noisy due to MEMS sensor characteristics. Dedicating separate capacity to encode these noisy signals without the denoising benefit of Kalman fusion actually hurts performance.
...
\begin{table}[H]
\caption{Comparison with prior work on IMU-based fall detection. $^\dagger$Results on SmartFallMM dataset. $^\ddagger$Results on comparable wrist-worn IMU datasets.}
\label{tab:sota}
\centering
\small
\begin{tabular}{llccl}
\toprule
\textbf{Method} & \textbf{Year} & \textbf{F1 (\%)} & \textbf{Acc (\%)} & \textbf{Evaluation} \\
\midrule
\multicolumn{5}{l}{\textit{SmartFallMM Dataset:}} \\
CNN-LSTM$^\dagger$ \cite{ngu2022personalized} & 2022 & 85.2 & 88.4 & 5-fold CV \\
Personalized CNN$^\dagger$ \cite{ngu2022personalized} & 2022 & 87.8 & 90.1 & LOSO \\
\textbf{Ours (Dual-Stream Kalman)$^\dagger$} & 2025 & \textbf{91.10} & \textbf{87.30} & 21-fold LOSO \\
\midrule
\multicolumn{5}{l}{\textit{Other Wrist-Worn IMU Datasets:}} \\
SVM + Handcrafted$^\ddagger$ & 2020 & 82.3 & 85.7 & LOSO \\
CNN-Transformer$^\ddagger$ & 2023 & 88.5 & 91.2 & 10-fold CV \\
ST-GCN (Skeleton)$^\ddagger$ & 2023 & 88.3 & 98.1 & Window-split \\
\bottomrule
\end{tabular}
\end{table}

Our dual-stream Kalman transformer achieves the highest reported F1 score on SmartFallMM (91.10\% vs. prior best 87.8\%). However, direct comparison requires caution: our evaluation uses 21-fold LOSO-CV on young subjects only, while prior work may have used different subject subsets or evaluation protocols. The stricter LOSO protocol better reflects real-world generalization to unseen individuals compared to random window splits or k-fold CV that may leak subject-specific patterns. We note that our accuracy (87.30\%) is lower than some prior methods; this reflects the challenging nature of strict subject-level evaluation and our focus on F1 score as the primary metric for imbalanced fall detection.
