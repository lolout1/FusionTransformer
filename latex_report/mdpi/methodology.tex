...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{preprocessing_pipeline.png}
\caption{Preprocessing pipeline for IMU data. Raw accelerometer and gyroscope signals are fused through a Linear Kalman Filter to produce stable orientation estimates (roll, pitch, yaw). The output 7-channel representation combines acceleration features with body pose information.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{arch_dual_stream_kalman.png}
\caption{Dual-stream Kalman transformer architecture. The accelerometer stream (SMV, $a_x$, $a_y$, $a_z$) and orientation stream ($\phi$, $\theta$, $\psi$) are processed through separate Conv1D projections with modality-specific normalization. Features are concatenated and passed through a transformer encoder with Squeeze-and-Excitation (SE) channel attention and Temporal Attention Pooling (TAP) for classification.}
