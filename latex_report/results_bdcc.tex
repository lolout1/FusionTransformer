...
\textbf{Dual-Stream Kalman} & 32:32 & \textbf{90.85} & \textbf{86.92} & 94.94 & \textbf{+1.05} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Kalman Fusion} & \textbf{Dual-Stream} & \textbf{90.85} & 94.94 & \textbf{+3.27} \\
\bottomrule
\end{tabular}
\end{table}
...
Kalman & 32:32 (balanced) & 64 & \textbf{90.85} & 94.94 \\
...
For Kalman-fused input, balanced allocation (32:32) with total dimension 64 achieves the best test F1 (90.85\%). Increasing total capacity to 96 dimensions does not improve performance, suggesting the model is not capacity-limited but rather benefits from the regularization effect of smaller embeddings.
...
\begin{enumerate}
    \item \textbf{Kalman fusion is essential}: Raw gyroscope input degrades performance due to noise contamination. Kalman fusion transforms this noisy signal into stable orientation estimates, enabling +3.27\% F1 improvement in dual-stream architectures.

    \item \textbf{Dual-stream requires quality input}: Dual-stream architecture only provides benefit when input streams are high-quality. With raw gyroscope, dual-stream hurts performance ($-$1.38\% F1); with Kalman fusion, it helps (+1.05\% F1).

    \item \textbf{Attention mechanisms complement architecture}: SE+TAP attention provides +1.22\% F1 on top of architectural improvements, with SE and TAP exhibiting synergy rather than independent contributions.
\end{enumerate}

The best configuration---dual-stream Kalman transformer with SE+TAP attention and 32:32 embedding split---achieves \textbf{90.85\% test F1} on 21-fold LOSO cross-validation.
