...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccc}
        \toprule
        \textbf{Model} & \textbf{Input} & \textbf{Test F1 [\%]} & \textbf{$\Delta$ F1} \\
        \midrule
        Acc-Only + SE & 4ch (smv, ax, ay, az) & 86.37 $\pm$ 8.24 & -- \\
        Acc+Gyro + SE & 6ch (all) & 84.75 $\pm$ 10.03 & $-$1.62\% \\
        \midrule
        \multicolumn{4}{l}{\textit{November 2025 results (no normalization):}} \\
...
\textbf{Finding:} Raw gyroscope fusion provides \textbf{no benefit} and slightly hurts performance ($-$1.6 to $-$2.0\% F1).
...
The following summarizes the three key comparisons emphasized throughout this study:

\begin{enumerate}
    \item \textbf{Dual-Stream vs Single-Stream:}
    \begin{itemize}
        \item With Kalman fusion: Dual-stream wins (+0.78\% to +1.67\% F1)
        \item Without Kalman: Marginal differences, single-stream slightly better
    \end{itemize}

    \item \textbf{Accelerometer vs Gyroscope Modalities:}
    \begin{itemize}
        \item Raw gyroscope: Hurts performance ($-$1.6\% to $-$2.0\% F1)
        \item Kalman-fused orientation: Helps significantly (+3.78\% F1)
    \end{itemize}

    \item \textbf{Kalman Fusion vs No Kalman:}
    \begin{itemize}
        \item Kalman fusion provides consistent improvement (+3.78\% F1)
        \item Linear KF preferred over EKF for simplicity (+0.59\% F1)
        \item Kalman \textit{smoothing} (filtering raw signals) hurts ($-$6.85\% F1)
    \end{itemize}
\end{enumerate}
