...
\begin{itemize}
    \item \textbf{Architecture:} Single-stream TransformerSE (1 encoder with SE attention)
...
\textbf{Experimental Setup:} All SE ablation comparisons use identical configurations except for the SE block:
\begin{itemize}
    \item \textbf{Architecture:} Single-stream Transformer (1 encoder)
...
\begin{table}[htbp]
    \centering
    \caption{Effect of SE attention on performance (accelerometer-only, single-stream architecture).}
    \label{tab:se_ablation}
    \begin{tabular}{lccccc}
        \toprule
        \textbf{Model} & \textbf{Input} & \textbf{SE Block} & \textbf{Test F1 [\%]} & \textbf{$\Delta$ F1} \\
        \midrule
        Transformer & 3ch acc & No & 76.29 $\pm$ 13.60 & -- \\
        Transformer + SMV & 4ch acc+smv & No & 81.28 $\pm$ 13.21 & +4.99\% \\
        \textbf{TransformerSE} & \textbf{3ch acc} & \textbf{Yes} & \textbf{86.37 $\pm$ 10.55} & \textbf{+10.08\%} \\
        TransformerSE + SMV & 4ch acc+smv & Yes & 85.54 $\pm$ 11.48 & +9.25\% \\
        \bottomrule
    \end{tabular}
\end{table}
...
\textbf{Key Findings:}
\begin{enumerate}
    \item SE blocks provide \textbf{+10.08\% F1 improvement} (76.29\% $\rightarrow$ 86.37\%) on accelerometer-only input
