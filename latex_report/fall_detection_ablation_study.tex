...
% ------------------------------------------------------------------
\begin{abstract}
This paper validates and extends our prior work on multimodal transformer architectures for wearable fall detection. Our November 2025 investigation identified that raw gyroscope fusion hurts performance (85.30\% vs 87.35\% F1 for accelerometer-only) and proposed Squeeze-and-Excitation (SE) channel attention, Temporal Attention Pooling (TAP), and dual-stream architectures as solutions. Through 12 controlled experiments on the SmartFallMM dataset (51 subjects: 30 young + 21 elderly), we validate these hypotheses with statistical rigor. Key findings: (i) \textbf{SE+TAP attention validated}---combined SE (channel weighting) and TAP (temporal focus) provide 10\% F1 improvement, 2.5$\times$ higher than the predicted 2-4\%; (ii) \textbf{Kalman fusion discovered}---in-model Kalman sensor fusion enables effective gyroscope utilization (+3-4\% F1), solving the raw fusion deficit; (iii) \textbf{dual-stream validated}---when combined with Kalman fusion, dual-stream \textbf{consistently outperforms} single-stream across both populations (+0.78\% Young+Old, +1.67\% Young-only with $p=0.074$). Our best configuration, Kalman Dual-Stream Balanced, achieves \textbf{91.01\% F1-score}---a 3.66\% absolute improvement over prior work. All experiments use per-sample z-score normalization and 22-fold Leave-One-Subject-Out cross-validation with paired t-tests.
\end{abstract}
...
\begin{itemize}
    ...
    \item \textbf{Source:} \texttt{saved\_exps/modality\_comparison/} (4-7 folds, preliminary)
\end{itemize}

\begin{table}[htbp]
    \centering
    \caption{Effect of input modality on fall detection (TransformerSE architecture, z-score normalized).}
    \label{tab:rq2_raw}
    \begin{tabular}{lcccc}
        \toprule
        \textbf{Model} & \textbf{Channels} & \textbf{Input Description} & \textbf{Test F1 [\%]} & \textbf{$\Delta$ F1} \\
        \midrule
        \textbf{TransformerSE (Acc)} & \textbf{3ch} & \textbf{ax, ay, az} & \textbf{85.39 $\pm$ 4.19} & \textbf{--} \\
        TransformerSE (Acc+SMV) & 4ch & smv, ax, ay, az & 82.19 $\pm$ 9.48 & $-$3.20\% \\
        TransformerSE (Acc+Gyro) & 6ch & ax, ay, az, gx, gy, gz & 81.51 $\pm$ 11.98 & $-$3.88\% \\
        TransformerSE (Full) & 7ch & smv, ax, ay, az, gx, gy, gz & 78.33 $\pm$ 16.07 & $-$7.06\% \\
        \bottomrule
    \end{tabular}
\end{table}

\textbf{Key Finding:} Adding gyroscope channels to raw accelerometer data \textbf{hurts} performance by $-$3.88\% F1. The best configuration uses \textbf{accelerometer-only (3 channels)}.
...
\begin{enumerate}
    \item \textbf{Gyroscope noise:} Raw gyroscope signals are noisy and can corrupt useful accelerometer features during early fusion.
    \item \textbf{Overfitting risk:} More input channels (6ch $\rightarrow$ 7ch) increases model complexity without proportional information gain, leading to worse generalization.
    \item \textbf{Kalman filtering:} Produces smooth orientation estimates (roll, pitch, yaw) by optimally combining noisy gyroscope with stable accelerometer reference.
...
\textbf{Raw gyroscope fusion hurts performance} by $-$3.88\% F1 (acc-only 85.39\% $\rightarrow$ acc+gyro 81.51\%).

Adding more channels progressively degrades performance: 3ch (85.4\%) $>$ 4ch (82.2\%) $>$ 6ch (81.5\%) $>$ 7ch (78.3\%).

\textbf{However, Kalman-fused gyroscope DOES help} by providing orientation estimates (+2.7 to +4.4\% F1).

\textbf{Practical recommendation:} Use Kalman fusion to extract orientation from gyroscope, rather than raw gyroscope channels. More channels $\neq$ better performance.
\end{tcolorbox}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Raw gyroscope: Hurts performance ($-$3.88\% F1, 85.39\% $\rightarrow$ 81.51\%)
        \item More channels progressively hurts: 3ch $>$ 4ch $>$ 6ch $>$ 7ch
        \item Kalman-fused orientation: Helps significantly (+2.7\% to +4.4\% F1)
        \item \textit{Source:} saved\_exps/modality\_comparison (4-7 folds, preliminary)
    \end{itemize}
